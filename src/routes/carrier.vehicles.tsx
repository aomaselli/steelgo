    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <Button onClick={() => setOpen(true)}>
          <Plus className="w-4 h-4" /> {t("carrierFleet.addTruck")}
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          tone="light"
          icon={TruckIcon}
          title={t("carrierFleet.emptyTitle")}
          description={t("carrierFleet.emptyDesc")}
          ...
          {trucks.map((truck) => (
            <Card key={truck.id} className="p-4 space-y-2 border-[#E3EAF3] bg-white">
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-lg font-bold text-[#10274A] tracking-wider">
                    {truck.plate ?? "—"}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-[#E3EAF3]">
                <div>
                  <div className="text-[#5B6B80]">{t("carrierFleet.type")}</div>
                  <div className="text-[#10274A] capitalize">{truck.type ?? "—"}</div>
                </div>
                <div>
                  <div className="text-[#5B6B80]">{t("carrierFleet.capacity")}</div>
                  <div className="text-[#10274A] tabular-nums">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={t("carrierFleet.modalTitle")}>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs text-[#5B6B80] block mb-1">
              {t("carrierFleet.capacity")}
            </label>
            <Input
              type="number"
