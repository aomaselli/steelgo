...
export function AdminESGPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-[#10274A]">ESG — Plataforma</h1>

      <div className="grid gap-4 md:grid-cols-4">
        <MetricCard icon={<Leaf className="h-5 w-5" />} label="CO₂ evitado total" value={`${fmtN(data?.totalSaved ?? 0)} kg`} color="text-[#1A9B5E]" />
        <MetricCard icon={<Truck className="h-5 w-5" />} label="Fretes verdes" value={fmtN(data?.greenCount ?? 0)} sub={`${greenPct.toFixed(1)}% do total`} color="text-[#1B6CB8]" />
        <MetricCard icon={<TreePine className="h-5 w-5" />} label="Árvores equivalentes" value={fmtN(data?.trees ?? 0)} color="text-[#1A9B5E]" />
        <MetricCard icon={<Trophy className="h-5 w-5" />} label="CO₂ emitido" value={`${fmtN(data?.totalEmitted ?? 0)} kg`} color="text-[#10274A]" />
      </div>

      <Card variant="light" className="p-5">
        <h3 className="mb-3 text-sm font-semibold text-[#10274A]">CO₂ emitido vs evitado — últimos 6 meses</h3>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data?.chartData ?? []}>
            <CartesianGrid stroke="#E6EDF3" strokeDasharray="3 3" />
            <XAxis dataKey="month" stroke="#54657C" fontSize={11} />
            <YAxis stroke="#54657C" fontSize={11} />
            <Tooltip contentStyle={{ background: "#FFFFFF", border: "1px solid #DDE7F2", borderRadius: 8, color: "#10274A" }} />
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card variant="light" className="p-5">
          <h3 className="mb-4 text-sm font-semibold text-[#10274A]">🏭 Top embarcadores por CO₂ evitado</h3>
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#E6EDF3]">
              {data?.topCompanies.length ? data.topCompanies.map((c, i) => (
                <tr key={c.id}>
                  <td className="py-2 text-[#54657C]">{i + 1}</td>
                  <td className="py-2 text-[#10274A]">{c.name}</td>
                  <td className="py-2 text-right tabular-nums text-[#1A9B5E]">{fmtN(c.saved)} kg</td>
                </tr>
              )) : (
                <tr><td colSpan={3} className="py-6 text-center text-[#54657C]">Sem dados</td></tr>
              )}
            </tbody>
          </table>
        </Card>
        ...
        <Card variant="light" className="p-5">
          <h3 className="mb-4 text-sm font-semibold text-[#10274A]">🚛 Top transportadoras por score ESG</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase text-[#54657C]">
                <th className="py-2">#</th>
            ...
            <tbody className="divide-y divide-[#E6EDF3]">
              {data?.topCarriers.length ? data.topCarriers.map((c, i) => (
                <tr key={c.id}>
                  ...
                  <td className="py-2 text-right tabular-nums text-[#54657C]">{c.overall.toFixed(1)}</td>
                </tr>
              )) : (
                <tr><td colSpan={4} className="py-6 text-center text-[#54657C]">Sem dados</td></tr>
              )}
            </tbody>
          </table>
        </Card>
...
function MetricCard({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string; sub?: string; color: string }) {
  return (
    <Card variant="light" className="p-4">
      <div className="flex items-center gap-2 text-[#54657C]">{icon}<span className="text-xs uppercase">{label}</span></div>
      <div className={`mt-2 text-3xl font-bold tabular-nums ${color}`}>{value}</div>
      {sub && <div className="mt-1 text-xs text-[#54657C]">{sub}</div>}
    </Card>
  );
}
