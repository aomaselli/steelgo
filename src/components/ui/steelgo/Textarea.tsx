...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(function Textarea(
  ...
  const base =
    'w-full bg-[#21262D] border rounded-[10px] px-3 py-2.5 text-[#E6EDF3] text-sm placeholder:text-[#484F58] outline-none transition-all resize-y min-h-[80px]';
  const state = error
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="relative flex">
        {leftIcon && (
          <span className="absolute left-3 top-3 text-[#484F58] text-base pointer-events-none">
            {leftIcon}
          </span>
        )}
        <textarea
        ...
        />
        {rightIcon && (
          <span className="absolute right-3 top-3 text-[#484F58]">{rightIcon}</span>
        )}
      </div>
