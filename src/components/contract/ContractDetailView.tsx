...
const CLAUSES = [
  {
    title: "Responsabilidade RCTR-C",
    body: "A transportadora é responsável pela carga conforme o seguro RCTR-C contratado, cobrindo perdas e avarias durante o transporte rodoviário até a entrega no destino indicado neste contrato.",
  },
  {
    title: "Penalidade por atraso",
    body: "Multa de 2% ao dia sobre o valor do frete por atraso injustificado na coleta ou entrega, limitado a 30% do valor total. Atrasos comunicados com antecedência de 6 horas não incorrem em multa.",
  },
  {
    title: "Condições de pagamento",
    body: "O pagamento ficará retido protegido até confirmação de entrega pelo embarcador. A liberação ocorre em até 24h úteis após a confirmação, descontada a taxa de plataforma de 3,5%.",
  },
  {
    title: "Foro",
    body: "Fica eleito o foro da Comarca de São Paulo, SP para dirimir quaisquer controvérsias decorrentes deste contrato, com renúncia expressa a qualquer outro.",
  },
];
...
function Banner({
  ...
  return (
    <div className={`flex items-center gap-2 border-b px-6 py-3 text-sm font-medium ${map[color]}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <li className="flex gap-3 pb-3 last:pb-0">
      <div className="flex flex-col items-center">
        <div
          className={`w-3 h-3 rounded-full ${done ? "bg-[#1A9B5E]" : current ? "bg-[#1B6CB8] animate-pulse" : "bg-[#DDE7F2] border border-[#C9D6E6]"}`}
        />
        <div className="w-px flex-1 bg-[#DDE7F2] mt-1" />
      </div>
      <div className="flex-1">
        <div className={`text-sm ${done || current ? "text-[#10274A]" : "text-[#54657C]"}`}>
          {label}
        </div>
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [freightRes, shipperRes, carrierRes, truckRes, driverRes] = await Promise.all([
        c.freight_id
          ? supabase.from("freights").select("*").eq("id", c.freight_id).maybeSingle()
          : Promise.resolve({ data: null }),
        c.shipper_company_id
          ? supabase
              .from("companies")
              .select("id, name, trade_name, cnpj")
              .eq("id", c.shipper_company_id)
              .maybeSingle()
          : Promise.resolve({ data: null }),
        c.carrier_company_id
          ? supabase
              .from("companies")
              .select("id, name, trade_name, cnpj")
              .eq("id", c.carrier_company_id)
              .maybeSingle()
          : Promise.resolve({ data: null }),
        c.truck_id
          ? supabase
              .from("trucks")
              .select("plate, type, capacity_tons")
              .eq("id", c.truck_id)
              .maybeSingle()
          : Promise.resolve({ data: null }),
        c.driver_id
          ? supabase
              .from("drivers")
              .select("full_name, cpf, license_number, license_category")
              .eq("id", c.driver_id)
              .maybeSingle()
          : Promise.resolve({ data: null }),
      ]);
  ...
  const { data: carrierMeta } = useQuery({
    ...
    queryFn: async () => {
      const { data: car } = await supabase
        .from("carriers")
        .select("antt_rntrc")
        .eq("company_id", data!.carrier_company!.id)
        .maybeSingle();
      return car;
    },
  });
  ...
  const banner = useMemo(() => {
    if (status === "awaiting_shipper_signature") {
      return isShipperOwner ? (
        <Banner color="amber">
          <Pen className="w-4 h-4" /> Aguardando sua assinatura
        </Banner>
      ) : (
        <Banner color="amber">
          <Pen className="w-4 h-4" /> Aguardando assinatura do embarcador
        </Banner>
      );
    }
    if (status === "awaiting_carrier_signature") {
      return isCarrierOwner ? (
        ...
        <Banner color="amber">
          <Pen className="w-4 h-4" /> Aguardando assinatura da transportadora
        </Banner>
      );
    }
    if (status === "active")
      return (
        <Banner color="blue">
          <Truck className="w-4 h-4" /> Contrato ativo — em execução
        </Banner>
      );
    if (status === "completed")
      return (
        <Banner color="green">
          <Check className="w-4 h-4" /> Contrato concluído
        </Banner>
      );
    if (status === "disputed")
      return (
        <Banner color="red">
          <AlertTriangle className="w-4 h-4" /> Em disputa — aguardando resolução
        </Banner>
      );
    return null;
        ...
        <div className="space-y-6 min-w-0">
          <Card variant="light" className="p-6">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <div className="font-mono text-xl font-bold text-[#10274A]">
                  {data.contract_number ?? `SG-${String(data.id).slice(0, 8).toUpperCase()}`}
                </div>
            ...
            <div className="border-t border-[#E6EDF3] mt-4 pt-2">
              ...
              <Row label="Caminhão">
                ...
                {truck?.capacity_tons ? ` • ${formatNum(truck.capacity_tons)} t` : ""}
              </Row>
              <Row label="Tipo de aço">
              ...
              <Row label="Rota">
                ...
                {f?.distance_km ? `· ${formatNum(f.distance_km)} km` : ""}
              </Row>
              <Row label="Data de coleta">
                {f?.pickup_date ? new Date(f.pickup_date).toLocaleDateString("pt-BR") : "—"}
                {data.pickup_window ? ` · ${data.pickup_window}` : ""}
              </Row>
              <Row label="Valor do frete">
                <span className="font-bold text-[#10274A]">
                  {formatBRL(
                    data.total_amount_brl
                      ? data.total_amount_brl - (data.platform_fee_brl ?? 0)
                      : f?.final_price_brl,
                  )}
                </span>
              </Row>
              <Row label="Taxa plataforma (3,5%)">
                <span className="text-[#54657C]">{formatBRL(data.platform_fee_brl)}</span>
              </Row>
              <Row label="Total a pagar">
                <span className="text-lg font-bold text-[#1B4F86]">
                  {formatBRL(data.total_amount_brl)}
                </span>
              </Row>
              <Row label="Categoria">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${isGreen ? "bg-[#EAF7F1] text-[#14603F]" : "bg-[#EEF3F8] text-[#54657C]"}`}
                >
                  {isGreen ? "🌿 Verde" : "Tradicional"}
                </span>
              </Row>
              {isGreen && (
                <Row label="CO₂ estimado">
                  <span className="text-[#1A9B5E]">
                    ~{Math.round((f?.weight_tons ?? 0) * (f?.distance_km ?? 0) * 0.018)} kg (frete
                    verde)
                  </span>
                </Row>
              )}
            ...
            <button
              ...
              className="mt-4 text-sm text-[#54657C] hover:text-[#10274A] flex items-center gap-1"
            >
              {showClauses ? (
                <>
                  Fechar cláusulas <ChevronUp className="w-3 h-3" />
                </>
              ) : (
                <>
                  Ver cláusulas contratuais <ChevronDown className="w-3 h-3" />
                </>
              )}
            </button>
            {showClauses && (
              <div className="mt-3 space-y-3 text-xs text-[#54657C] border-t border-[#E6EDF3] pt-3">
                {CLAUSES.map((cl, i) => (
                  <div key={i}>
                    <div className="font-semibold text-[#10274A]">
                      {i + 1}. {cl.title}
                    </div>
          ...
          <Card variant="light" className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <SignatureBox
                ...
                pad={<SignaturePad contractId={contractId} party="carrier" onSigned={refetchAll} />}
              />
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-4 lg:sticky lg:top-4 self-start">
          <Card variant="light" className="p-0 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-[#DDE7F2]">
              ...
              {data.pdf_url && (
                <a href={data.pdf_url} download className="text-[#54657C] hover:text-[#10274A]">
          ...
          <Card variant="light" className="p-5">
            ...
            <ol className="space-y-0">
              ...
              <TimelineStep
                done={!!data.shipper_signed_at}
                current={status === "awaiting_shipper_signature"}
                label="Assinatura do embarcador"
                ts={data.shipper_signed_at}
              />
              <TimelineStep
                done={!!data.carrier_signed_at}
                current={status === "awaiting_carrier_signature"}
                label="Assinatura da transportadora"
                ts={data.carrier_signed_at}
              />
              <TimelineStep
                done={
                  data.escrow_status === "funding_confirmed" ||
                  data.escrow_status === "released_confirmed"
                }
                current={status === "active" && data.escrow_status === "awaiting_funding"}
                label="Pagamento confirmado"
                ts={data.escrow_held_at}
              />
              <TimelineStep
          ...
          {(status === "active" || status === "completed") && (
            <Card variant="light" className="p-5 space-y-3">
              ...
              <div className="text-2xl font-bold text-[#10274A] tabular-nums">
                {formatBRL(data.total_amount_brl)}
              </div>
              ...
              {isShipperOwner && data.escrow_status === "pending" && (
                <Link to="/shipper/payment/$contractId" params={{ contractId }}>
                  <Button className="w-full">Solicitar pagamento protegido →</Button>
                </Link>
              )}
              {isShipperOwner && data.escrow_status === "funding_confirmed" && (
                <Button variant="green" className="w-full" onClick={() => setReleaseOpen(true)}>
                  Confirmar entrega e solicitar liberação →
                </Button>
              )}
            </Card>
          )}
        ...
        <>
          {sigUrl && (
            <img
              src={sigUrl}
              alt="assinatura"
              className="w-full h-24 object-contain rounded-[8px] bg-[#F7F9FB] mt-2"
            />
          )}
          <div className="text-sm text-[#1A9B5E] mt-2">
